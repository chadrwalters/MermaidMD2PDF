...
def test_complete_workflow(temp_dir, sample_markdown):
    ...
    for diagram in diagrams:
        image_path = temp_dir / f"diagram_{diagram.line_number}.png"
        image_generator.generate_image(diagram.content, image_path)
        assert image_path.exists()
        image_paths.append(image_path)

    # Step 5: Generate PDF
    pdf_generator = PDFGenerator()
    pdf_generator.generate_pdf(input_file, output_file, diagrams, image_paths)
...
"""
    ...
    with pytest.raises(Exception):
        image_generator.generate_image(diagrams[0].content, temp_dir / "error.png")
